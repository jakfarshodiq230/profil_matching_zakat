      ...
      <div class="col-md-10">
        <div class="box box-primary">
          <div class="box-header with-border">
            <h3 class="box-title">Edit Kandidat Calon Penerima Zakat</h3>
          </div>
          <!-- /.box-header -->
          ...
          <div class="box-body">
            ...
            <form class="form-horizontal" action="<?= base_url('admin/kandidatEdit/').$this->encrypt->encode($onekandidat->id_kandidat).''?>" method="post" role="form" >
              ...
              <div class="form-group">
                <label for="editInputNama" class="col-sm-4 control-label">Nama Calon Penerima Zakat</label>
                <div class="col-sm-8">
                  <select class="form-control select2" id="editInputNama" name="nik">
                    <?php foreach($pegawai as $row) :?>
                      <?php if($row->nik == $onekandidat->nik) :?>
                        <option value="<?= $row->nik?>" selected><?= $row->nama_penerima;?></option>
                        <?php else :?>
                    ...
                    <div class="col-sm-8">
                      <select class="form-control select2" name="faktor[<?= $fak->kode_faktor?>]" required>
                        ...
                        <?php foreach($skala as $key => $value) : ?>
                          <?php $sel = ($key == $onekandidat->nilai_faktor) ? ' selected' : '' ?>
